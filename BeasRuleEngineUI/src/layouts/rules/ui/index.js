...
const { default: RuleModel, default: DataModel } = require("../model");

function RulesUI({ children }) {
    ...
    const createSubmitAction = (formJson) => {
        postData("/rule-library/create", {
            "name": formJson.name,
            "description": formJson.description,
            "mvlCode": formJson.mvlCode,
            "functions": formJson.functions.split(",").map((f) => f.trim()),
            "helpers": formJson.helpers.split(",").map((h) => h.trim()),
    ...
    const handleDelete = async (item) => {
        return new Promise((resolve) => {
            confirm({ description: `This will permanently delete ${item.id}.` })
                .then(() => {
                    deleteData("/rule-library/delete/" + item.id).then(() => resolve());
                })
                .then(() => {
                    DataModel
